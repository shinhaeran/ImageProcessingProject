...
void Compare(int x, int y, int** block_out[], int bsize, int** img, int** img_out) {
	...
	for (int i = 0; i < 510; i++) {
		err = ComputeError(x, y, img, block_out[i], bsize,768); // 원래 img랑 회전들이랑 error를 계산해서

		if (err < err_min) {//가장 error가 적은 index를 찾고
			err_min = err;
			index_min = i;
		}
	}
	...
	for (int Y = 0; Y < bsize; Y++) {
		for (int X = 0; X < bsize; X++) {
			img_out[Y + y][X + x] = block_out[index_min][Y][X]; //결과 img_out에 블락을 넣어줌
		}
	}
}
void MultiTemplate(int height, int width, int** img, int** img_out, int** block_out[], int bsize) { // block_out의 블럭들을 bsize만큼 크기로 넣어줌
	...
	for (int y = 0; y < height; y += 32) {
		for (int x = 0; x < width; x += 32) {
			Compare(x, y, block_out, bsize, img, img_out);//img랑 block_out이랑 비교해서 img_out에 넣어주는거.
		}
	}
}
...
void Compare_color(int x, int y, int_rgb** block_out[], int bsize, int_rgb** img, int_rgb** img_out) {
	...
	for (int i = 0; i < 510; i++) {
		err = ComputeError_color(x, y, img, block_out[i], bsize, 768); // 원래 img랑 회전들이랑 error를 계산해서
		if (err < err_min) {//가장 error가 적은 index를 찾고
			err_min = err;
			index_min = i;
		}
	...
	for (int Y = 0; Y < bsize; Y++) {
		for (int X = 0; X < bsize; X++) {
			img_out[Y + y][X + x].r = block_out[index_min][Y][X].r; //결과 img_out에 블락을 넣어줌
...
void MultiTemplate_color(int height, int width, int_rgb** img, int_rgb** img_out, int_rgb** block_out[], int bsize) { // block_out의 블럭들을 bsize만큼 크기로 넣어줌

	for (int y = 0; y < height; y += 32) {
		for (int x = 0; x < width; x += 32) {
			Compare_color(x, y, block_out, bsize, img, img_out);//img랑 block_out이랑 비교해서 img_out에 넣어주는거.
		}
	}
}


void ReadBlock_img1_color(int_rgb **block, int_rgb** img, int width, int height) {//+90도->상하대칭됨;
	int indexB = 0;
...
void ReadBlock_img2_color(int_rgb **block, int_rgb** img, int width, int height) {//-90도-> 어 좌우대칭됨;
	int indexB = 0;
...
void ReadBlock_img3_color(int_rgb **block, int_rgb** img, int width, int height) {//좌우대칭
	int indexB = 0;
...
void ReadBlock_img4_color(int_rgb **block, int_rgb** img, int width, int height) {//상하대칭(ㅇ)
	int indexB = 0;
...
void main() {
	...
	int_rgb** img_out1 = IntColorAlloc2(width,height); //회전x
	int_rgb** img_out2 = IntColorAlloc2(width, height); //회전o
	...
	char filename[100];
	int bsize = 32;
	int_rgb** block[510];//510개 템플릿
	int_rgb** block_out[510 * 5];//회전시킨거 차례대로 넣을거
	...
	for (int y = 0; y < height; y += 32) {
		for (int x = 0; x < width; x += 32) {
			...
			for (int i = 0; i <510*5; i++) {
				...
				err = ComputeError_color(x, y, img, block_out[i], bsize, height); // 해당 블럭의 찾기
				printf("%d \n", err);
				if (err < err_min) {//min - out 비교
					err_min = err;
					index_min = i;
	...
	//ColorImageShow("회전X", img_out1, width, height);
	//ColorImageShow("회전O", img_out2, width, height);
	waitKey(0);
}
